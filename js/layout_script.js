...
const bm_Ca_slide = document.getElementById("bm_Ca_slide"),
  ...
  bm_randomStartValue_checkbox = document.getElementById(
    "bm_randomStartValueCheckbox",
  ),
...
bm_RESET_BTN.onclick = function () {
  ...
  bm_SLIDER.forEach((element, index) => {
    // reset the sliders
  ...
  bm_value_fields.forEach((element, index) => {
    // Reset value fields
    const default_value = window.bm_default_input[bm_plot_variables[index]];
...
for (let entry = 0; entry < bm_SLIDER.length; entry++) {
  bm_SLIDER[entry].oninput = function () {
    ...
    document.getElementById(elem_id + "value").innerHTML =
      document.getElementById(bm_SLIDER[entry].id).value;
  };
...
const bm_dens_Ca_slide = document.getElementById("bm_dens_Ca_slide"),
  ...
  bm_dens_randomStartValue_checkbox = document.getElementById(
    "bm_dens_randomStartValueCheckbox",
  ),
...
bm_dens_RESET_BTN.onclick = function () {
  ...
  bm_dens_SLIDER.forEach((element, index) => {
    // reset the sliders
    const default_value =
      window.bm_default_input[bm_dens_plot_variables[index]];
    document.getElementById(element.id).value = default_value;
  });
  bm_dens_value_fields.forEach((element, index) => {
    // Reset value fields
    const default_value =
      window.bm_default_input[bm_dens_plot_variables[index]];
    document.getElementById(element.id).innerHTML = default_value;
...
for (let entry = 0; entry < bm_dens_SLIDER.length; entry++) {
  bm_dens_SLIDER[entry].oninput = function () {
    ...
    elem_id = elem_id.substring(0, elem_id.length - 5);
    document.getElementById(elem_id + "value").innerHTML =
      document.getElementById(bm_dens_SLIDER[entry].id).value;
  };
...
df_RESET_BTN.onclick = function () {
  ...
  df_SLIDER.forEach((element, index) => {
    // reset the sliders
    const default_value = window.bm_default_input[df_plot_variables[index]];
    document.getElementById(element.id).value = default_value;
  });
  df_value_fields.forEach((element, index) => {
    // Reset value fields
    const default_value = window.bm_default_input[df_plot_variables[index]];
    document.getElementById(element.id).innerHTML = default_value;
  });
...
for (let entry = 0; entry < df_SLIDER.length; entry++) {
  df_SLIDER[entry].oninput = function () {
    ...
    elem_id = elem_id.substring(0, elem_id.length - 5);
    document.getElementById(elem_id + "value").innerHTML =
      document.getElementById(df_SLIDER[entry].id).value;
  };
